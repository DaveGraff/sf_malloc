...
Test(sf_memsuite_student, malloc_simple, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(double) * 8);

	cr_assert_not_null(x, "x is NULL!");
}

Test(sf_memsuite_student, malloc_simple2, .init = sf_mem_init, .fini = sf_mem_fini) {
	int *x = sf_malloc(sizeof(int));
	*x = 2;

	cr_assert_eq(*x, 2, "x has the wrong value!");
}

Test(sf_memsuite_student, malloc_fail, .init = sf_mem_init, .fini = sf_mem_fini) {
	int *x = sf_malloc(0);

	cr_assert_eq(x, NULL, "x has the wrong value!");
}

Test(sf_memsuite_student, realloc_fail, .init = sf_mem_init, .fini = sf_mem_fini) {
	int *x = sf_realloc(0, 0);
	printf("%p\n", x);

	cr_assert_eq(x, NULL, "x has the wrong value!");
	cr_assert_eq(sf_errno, EINVAL, "sf_errno not set!");
}

Test(sf_memsuite_student, malloc_simple3, .init = sf_mem_init, .fini = sf_mem_fini) {
	int *x = sf_malloc(sizeof(int));
	*x = 4;
	cr_assert_eq(*x, 4, "x has the wrong value!");
	sf_free(x);
}